...
| `bas_fmplaystring` | `$C006` | BASIC | Plays a note script using the FM channel which was specified on a previous call to `bas_playstringvoice`. This string pointer must be in low RAM (`$0000`-`$9EFF`). | .A = string length <br/> .X .Y = pointer to string | none | none |
...
| `bas_psgplaystring` | `$C018` | BASIC | Plays a note script using the PSG voice which was specified on a previous call to `bas_playstringvoice`. This string pointer must be in low RAM (`$0000`-`$9EFF`). | .A = string length <br/> .X .Y = pointer to string | none | none |
...
| `ym_loadpatch` | `$C069` | YM2151 | Load patch by number from the audio bank, or from an arbitrary memory location into a channel. High RAM addresses (`$A000`-`$BFFF`) are accepted. | .A = channel<br/>.C clear = .X .Y = patch address<br/>.C set = .X = patch number | .C clear = success <br/> .C set = error | none |
...
| `ym_write` | `$C08A` | YM2151 | Write a value to one of the YM2151 registers and to the in-RAM shadow copy. If the selected register is a TL register, attenuation will be applied before the value is written. If the selected register is RLFBCON and the CON portion changes, the TL registers for M1 M2 and C1 are rewritten with attenuation applied as appropriate so that attenuation is only applied to operators now designated as carriers | .A = value <br/>.X = YM register address | .C clear = success <br/> .C set = error | .A .X |

## Direct communication with the YM2151 and VERA PSG vs API

Use of the API routines above are not required to access the capabilities of the sound chips. However, mixing raw writes to a chip and API access for the same chip is not recommended, particularly where PSG volumes and YM2151 TL and RLFBCON registers are concerned. The API processes volumes, calculating attenuation and adjusting the output volume accordingly, and the API will be oblivious to direct manipulation of the sound chips.
